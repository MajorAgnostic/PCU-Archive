...
; Kanto hidden items
	...
	const EVENT_ROUTE_2_HIDDEN_BIGMUSHROOM
	...
	const EVENT_HOMEPIKAPOSTER ; Unused: next 339 events starting here
	const EVENT_HOMEPPUP
	const EVENT_HOMESNES
	const EVENT_HOMEVOLTORBDOLL
	const EVENT_HOMEUPGRADE
	const EVENT_HOMEBIGLAPRASDOLL
	const EVENT_HOMEBRICKPIECE
	const EVENT_HOMEBIGONIXDOLL
	const EVENT_HOMEUSELESS7
	const EVENT_HOMEUSELESS8
	const EVENT_BEAT_VRBRADY
	const EVENT_GOT_POCKETPC
	const EVENT_ROUTE_40_HIDDEN_SOFT_SAND
	const EVENT_SLOWPOKE_WELL_B2F_SLOWPOKETAIL
	const EVENT_SLOWPOKE_WELL_B1F_HIDDEN_SLOWPOKETAIL
	const EVENT_BERSERKGENE_REPLICATION
	const EVENT_MET_BLUE_IN_CINNABAR
	const EVENT_GOT_THICKCLUB
	const EVENT_ILEX_FOREST_HIDDEN_BIGMUSHROOM
	const EVENT_ILEX_FOREST_HIDDEN_TINYMUSHROOM
	const EVENT_ILEX_FOREST_HIDDEN_TINYMUSHROOM2
	const EVENT_ROUTE_40_HIDDEN_STAR_PIECE
	const EVENT_ROUTE_41_HIDDEN_WATER_STONE
	const EVENT_BEAT_SWIMMERF_CLAIRE
	const EVENT_MT_SILVER_MASTER_BALL
	const EVENT_ROUTE_28_MAX_REVIVE
	const EVENT_TIN_TOWER_1F_MORTY
	const EVENT_ILEX_LUCKY_EGG
	const EVENT_ZAPDOS_STATION
	const EVENT_TRAIN_STATION_MAGNET
	const EVENT_BEAT_ANDREA
	const EVENT_NO_E4_REMATCH
	const EVENT_GYM_LEFTOVERS
	const EVENT_GOT_ROUTE2SHARE
	const EVENT_ROUTE_2_BIGMUSHROOM
	const EVENT_BOULDER_IN_SEAFOAM_1
	const EVENT_BOULDER_IN_SEAFOAM_2
	const EVENT_BOULDER_IN_SEAFOAM_3
	const EVENT_BOULDER_IN_SEAFOAM_4
	const EVENT_BOULDER_IN_SEAFOAM_5
	const EVENT_BOULDER_IN_SEAFOAM_6
	const EVENT_BOULDER_IN_SEAFOAM_7
	const EVENT_BOULDER_IN_SEAFOAM_8
	const EVENT_BOULDER_IN_SEAFOAM_9
	const EVENT_BOULDER_IN_SEAFOAM_10
	const EVENT_BOULDER_IN_SEAFOAM_11
	const EVENT_BOULDER_IN_SEAFOAM_12
	const EVENT_BOULDER_IN_SEAFOAM_13
	const EVENT_BOULDER_IN_SEAFOAM_14
	const EVENT_SEAFOAM_ICE_HEAL
	const EVENT_SEAFOAM_HIDDEN_MAX_REVIVE
	const EVENT_SEAFOAM_WATER_STONE
	const EVENT_SEAFOAM_HIDDEN_BIG_PEARL
	const EVENT_SEAFOAM_HIDDEN_NUGGET
	const EVENT_SEAFOAM_HIDDEN_MAX_ELIXER
	const EVENT_SEAFOAM_HIDDEN_ULTRA
	const EVENT_SEAFOAM_NEVERMELTICE
	const EVENT_ARTICUNO
	const EVENT_MOLTRES
	const EVENT_SPOKE_EUSINE
	const EVENT_EUSINE_LEAVES
	const EVENT_TIN_MORTY
	const EVENT_BEAT_BLACKBELT_KIYO2
	const EVENT_DOJO_KIYO
	const EVENT_GOT_DOJOPOKE
	const EVENT_ROUTE26_LEFTOVERS
	const NOMOLEFTOVERS
	const EVENT_BEAT_JANET
	const EVENT_GOT_MOM_GIFT_CALL
	; New generic Kanto/post-game trainers and items start here
	const EVENT_FAST_SHIP_MAX_POTION
	const EVENT_FAST_SHIP_MAX_ETHER
	const EVENT_FAST_SHIP_REVIVE
	const EVENT_ROUTE_7_LEAF_STONE
	const EVENT_CINNABAR_FIRE_STONE
	const EVENT_BEAT_SELINA
	const EVENT_BEAT_DAY
	const EVENT_BEAT_VERGIL
	const EVENT_CELADON_HIDDEN_MAX_ELIXIR
	const EVENT_BEAT_LULU
	const EVENT_BEAT_TOD
	const EVENT_BEAT_CONNER
	const EVENT_BEAT_DREW
	const EVENT_BEAT_ALICIA
	const EVENT_ROUTE_9_FULL_RESTORE
	const EVENT_ROUTE_10_THUNDERSTONE
	const EVENT_BEAT_POKEMANIAC_WINSTON
	const EVENT_BEAT_POKEMANIAC_STEVE
	const EVENT_BEAT_SOFIA
	const EVENT_BEAT_LENNY
	const EVENT_BEAT_LEAH
	const EVENT_BEAT_SEKHER
	const EVENT_BEAT_OFFICER_GRANT
	const EVENT_BEAT_LUCA
	const EVENT_BEAT_KYLER
	const EVENT_BEAT_CAMPER_JUSTIN
	const EVENT_ADAM_APPEARS
	const EVENT_BEAT_DAL
	const EVENT_BEAT_REESE
	const EVENT_BEAT_DAN
	const EVENT_BEAT_TEDDY
	const EVENT_BEAT_MARKEY
	const EVENT_BEAT_JACOB
	const EVENT_BEAT_THERON
	const EVENT_BEAT_PIPER
	const EVENT_BEAT_GINGER
	const EVENT_BEAT_BOONE
	const EVENT_BEAT_CONNOR
	const EVENT_BEAT_CLARICE
	const EVENT_BEAT_IDAIN
	...
	const EVENT_ROUTE_2_BIGMUSHROOM2
	const EVENT_BEAT_ANDER
	const EVENT_BEAT_ANTONIA
	const EVENT_BEAT_DANTE
	const EVENT_PEWTER_HIDDEN_FR
	const EVENT_BEAT_SHERMAN
	const EVENT_BEAT_FRENCH
	const EVENT_BEAT_KINSLEY
	const EVENT_BEAT_MINA
	const EVENT_BEAT_FRANKIE
	const EVENT_BEAT_MURPHY
	const EVENT_QUIZ1
	const EVENT_QUIZ1_WRONG
	const EVENT_QUIZ2
	const EVENT_QUIZ2_WRONG
	const EVENT_QUIZ3
	const EVENT_QUIZ3_WRONG
	const EVENT_QUIZ4
	const EVENT_QUIZ4_WRONG
	const EVENT_QUIZ5
	const EVENT_QUIZ5_WRONG
	const EVENT_BEAT_LOWELL
	const EVENT_BEAT_DANIEL
	const EVENT_BEAT_CARY
	const EVENT_BEAT_LINDEN
	const EVENT_BEAT_OSCAR
	const EVENT_BEAT_WALDO
	const EVENT_BEAT_MERLE
	const EVENT_BEAT_EINSTEIN
	const EVENT_BEAT_DYLAN
	const EVENT_BEAT_TANNER
	const EVENT_BEAT_WILKINSON
	const EVENT_BEAT_DARIMI
	const EVENT_BEAT_CARL
	const EVENT_BEAT_CALLUM
	const EVENT_BEAT_JOEY6
	const EVENT_BEAT_PATRICK
	const EVENT_BEAT_LEROY
	const EVENT_BEAT_HARVEY
	const EVENT_BEAT_JOHN
	const EVENT_BEAT_SEPH
	const SEAFOAM_LANCE_READY
	const RED_REMATCH
	const EVENT_BEAT_SEAFOAM_LANCE
	const OFFERED_METAL_POWDER
...
; Bird Keeper
	...
	const EVENT_BEAT_BIRD_KEEPER_KIRT
...
; Boarder
	...
	const EVENT_BEAT_BOARDER_THEO
	const EVENT_BEAT_BOARDER_JORDAN
; Sage
...
; Biker
	...
	const EVENT_BEAT_BIKER_GRYPHON
; Psychic
...
; Picnicker
	...
	const EVENT_BEAT_PICNICKER_BETH
; Guitarist
...
; Blackbelt
	...
	const EVENT_BEAT_BLACKBELT_BOBBY
; Beauty
...
; Bug Catcher
	...
	const EVENT_BEAT_BUG_CATCHER_TOBIAS
...
; CooltrainerM
	...
	const EVENT_BEAT_COOLTRAINERM_CHRIST
; CooltrainerF
...
; Youngster
	...
	const EVENT_BEAT_YOUNGSTER_TAYLOR
; Teacher
...
; Johto people
	...
	const EVENT_ROUTE42ECRUTEAKGATE_OPEN ; 6 badges check complete
...
; New to Crystal
	...
	const EVENT_PICKED_UP_MIRACLEBERRY1_FROM_HO_OH_ITEM_ROOM
	const EVENT_PICKED_UP_MIRACLEBERRY2_FROM_HO_OH_ITEM_ROOM
	...
	const EVENT_MEW_IN_MT_SILVER ; Unused: next 48 events (new events start here (not counting regular trainers); they continue in "Kanto hidden items")
	const EVENT_FOUGHT_MEW
	const EVENT_GOT_MEWTWO
	const EVENT_GOT_KABUTO
	const EVENT_GOT_OMANYTE
	const EVENT_GOT_AERODACTYL
	const EVENT_BEAT_NICHOLAS
	const EVENT_VICTORY_ROAD_TM_STEEL_WING
	const EVENT_RUINS_OF_ALPH_OUTSIDE_TWISTEDSPOON
	const EVENT_ILEX_FOREST_SILVERPOWDER
	const EVENT_LUGIACHAMBER_SACREDASH
	const EVENT_OPENED_RIGHTBB
	const EVENT_GOT_UNOWN_DOLL
